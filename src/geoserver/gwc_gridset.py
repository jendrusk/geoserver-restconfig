...
from geoserver.support import (
    ResourceInfo, xml_property,
    write_bool, read_bool, read_string, string_list, read_int, write_string, write_int, write_string_list,
    read_extent, write_extent, read_float_list, write_float_list, read_float, write_float, read_srs, write_srs)


def gwcgridset_from_index(catalog, node):
    gridset_name = node.find("name").text
    return GridSet(catalog, gridset_name)

class GridSet(ResourceInfo):
    resource_type = "gridSet"
    save_method = "PUT"

    def __init__(self, catalog, gridset_name):
        super(GridSet, self).__init__()
        self._catalog = catalog
        self._gridset_name = gridset_name
        self.name = gridset_name
        self.write_all = True
    ...
    def gridset_name(self):
        return self._gridset_name

    @property
    def href(self):
        return "{gwc_url}/gridsets/{gridset_name}".format(gwc_url=self.catalog.gwc_url, gridset_name=self.gridset_name)

    name = xml_property('name', read_string)
    description = xml_property('description', read_string)
    srs = xml_property('srs', read_srs)
    extent = xml_property('extent', read_extent)
    alignTopLeft = xml_property('alignTopLeft', read_bool)
    scaleDenominators = xml_property('scaleDenominators', read_float_list)
    resolutions = xml_property('resolutions', read_float_list)
    metersPerUnit = xml_property('metersPerUnit', read_float)
    pixelSize = xml_property('pixelSize', read_float)
    scaleNames = xml_property('scaleNames', string_list)
    tileHeight = xml_property('tileHeight', read_int)
    tileWidth = xml_property('tileWidth', read_int)
    yCoordinateFirst = xml_property('yCoordinateFirst', read_bool)
    ...
    writers = {
        ...
        'alignTopLeft': write_bool('alignTopLeft'),
        'scaleDenominators': write_float_list('scaleDenominators'),
        'metersPerUnit': write_float('metersPerUnit'),
        'pixelSize': write_float('pixelSize'),
        'scaleNames': write_string_list('scaleNames'),
        'tileHeight': write_int('tileHeight'),
        'tileWidth': write_int('tileWidth'),
        'yCoordinateFirst': write_bool('yCoordinateFirst'),
        'resolutions': write_float_list('resolutions')
    }
